...
double simulated_annealing(CITY * tsp_prob,int cc, double init_temp)
{
	int nTry = 0, nAcc = 0; double t = init_temp, r, p, oldE, newE;
...
int main()
{
		...
		for(j = 0; j < 10; j++)
			sum += simulated_annealing(tsp_prob, 20000, 100);
		printf("\nThe average energy is (%f).\n\n\n", sum/10);
		return 0; 
}
